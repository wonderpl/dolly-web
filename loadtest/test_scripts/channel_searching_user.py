...
class Transaction(BaseTransaction):
    ...
    def process(self):
        ...
        - Search for channels with that term
        ...
        for page_count in xrange(random.randint(1, 10)):
            ...
            channels = self.get(self.urls['channel_search'], params=params)['channels']
            #for channel in channels['items'][:random.randint(0, min(10, len(channels)))]:
            ...
            #                  token=self.token)
            #        yield True
            if channels['total'] < (page_count + 1) * 100:
                break
